...
use App\Http\Exceptions\InvalidStatusReversal;
 ...
 *
 * @method static \Illuminate\Database\Eloquent\Builder|ModelStatus[] whereCurrentStatus($value)
 */
trait HasStatuses
{
     ...
     * @throws Exception
     */
    public function setStatus(?string $status, ?string $reason = null, $type = null): ModelStatus
    {
        if (!$this->id) {
    ...
    {
        return $this->morphMany(ModelStatus::class, 'model')
            ->orderBy('id', 'DESC');
    }

    public function currentPrimaryStatus(): MorphOne
    {
        return $this->morphOne(ModelStatus::class, 'model')
            ->where('type', 'primary')
            ->orderBy('id', 'DESC');
    }
    ...
    public function primaryStatus(): ?ModelStatus
    {
        $morphMany = $this->morphMany(ModelStatus::class, 'model')
            ->whereType('primary')
            ->orderBy('id', 'DESC');
    ...
    public function secondaryStatus(): ?ModelStatus
    {
        $morphMany = $this->morphMany(ModelStatus::class, 'model')
            ->whereType('secondary')
            ->orderBy('id', 'DESC');
